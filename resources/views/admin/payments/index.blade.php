        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.component-payments.index') }}" class="row g-3">
                ...
                <div class="col-md-2">
                    ...
                    <input type="date" name="date_from" class="form-control mb-1" value="{{ request('date_from') }}" placeholder="From">
                        ...
                        <tr>
                            <td class="font-weight-bold text-primary">
                                ...
                                @if($payment->transaction_id)
                                    <br><small class="text-muted" title="Transaction ID">TXN: {{ $payment->transaction_id }}</small>
                                @endif
                            </td>
                            <td>
                                @if($payment->student)
                                    <a href="{{ route('admin.students.show', $payment->student_id) }}" class="font-weight-bold text-dark">
                            ...
                            <td> {{-- [NEW COLUMN DATA] --}}
                                <div class="d-flex align-items-center">
                                    <div class="avatar-circle-sm bg-secondary text-white mr-2" style="width: 24px; height: 24px; font-size: 10px; display: flex; align-items: center; justify-content: center; border-radius: 50%;">
                                        {{ substr($payment->createdBy->name ?? 'S', 0, 1) }}
                                    </div>
                                    <span class="small font-weight-bold">{{ $payment->createdBy->name ?? 'System' }}</span>
                                </div>
                            </td>
                            <td>
                                    ...
                                    <ul class="list-unstyled mb-0 small">
                                        @foreach($payment->componentItems as $item)
                                            <li class="d-flex justify-content-between">
                                                ...
                                                <span class="font-weight-bold">₹{{ number_format($item->amount_paid, 0) }}</span>
                                            </li>
                                        @endforeach
                                    </ul>
                                @else
                            ...
                            <td>
                                <div class="h6 font-weight-bold text-success mb-0">₹{{ number_format($payment->amount, 2) }}</div>
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ route('admin.component-payments.show', $payment->id) }}" class="btn btn-outline-primary" title="View Details">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('public.receipt.show', $payment->receipt_number) }}" class="btn btn-outline-info" target="_blank" title="Print Receipt">
