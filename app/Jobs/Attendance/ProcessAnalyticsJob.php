...
class ProcessAnalyticsJob implements ShouldQueue
{
    ...
    private function calculateRiskScore($cache): float
    {
        ...
        if ($cache->attendance_percentage < 50) {
            $score += 40;
        } elseif ($cache->attendance_percentage < 65) {
            $score += 30;
        ...
        if ($cache->consecutive_absents >= 7) {
            $score += 25;
        } elseif ($cache->consecutive_absents >= 5) {
            $score += 20;
        } elseif ($cache->consecutive_absents >= 3) {
            $score += 15;
        } elseif ($cache->consecutive_absents >= 2) {
            $score += 10;
        }
        ...
        if ($cache->trend_direction === 'declining') {
            $score += 20;
        } elseif ($cache->trend_direction === 'stable' && $cache->attendance_percentage < 75) {
            $score += 10;
        } elseif ($cache->trend_direction === 'improving') {
            $score -= 5;
        }
