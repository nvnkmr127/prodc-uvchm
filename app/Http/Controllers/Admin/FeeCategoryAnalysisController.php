...
public function show(Request $request, $id)
    {
        ...
        return view('admin.fee-category-analysis.show', compact(
            'feeCategory', 'studentFees', 'batches', 'courses', 'stats'
        ));
    }
/**
...
public function pendingStudents(FeeCategory $feeCategory, Request $request)
{
    ...
    $pendingStudents = StudentFee::where('fee_category_id', $feeCategory->id)
        ...
        }, 'student.batch.course', 'feeCategory'])
...
private function getCategoryAnalysis($filters)
{
    ...
    return $query
        ...
        ->groupBy('fee_categories.id', 'fee_categories.name', 'fee_categories.description',
                 'fee_categories.category_code', 'fee_categories.category_type',
                 'fee_categories.is_mandatory', 'fee_categories.created_at', 'fee_categories.updated_at')
...
private function getStudentDetailsForCategory(FeeCategory $feeCategory, $filters)
{
    ...
    return $query
        ...
        ->groupBy('students.id', 'students.name', 'students.enrollment_number', 'students.email',
                  'students.batch_id', 'batches.name', 'courses.name')
...
private function getStudentsForCategoryReminders($feeCategory, $criteria)
{
    ...
    return $query->with(['studentFees' => function($q) use ($feeCategory) {
        $q->where('fee_category_id', $feeCategory->id)
          ->whereIn('status', ['unpaid', 'partial']);
    }])->get();
}
...
private function getRecoveryOpportunities($filters) { return []; }
private function calculateAverageRecoveryDays($filters) { return 30; }
