...
use App\Models\Attendance;  // ✅ FIXED: Correct namespace
...
use Illuminate\Support\Facades\DB;  // ✅ ADD: Missing import
...
public function testETimeOfficeConnection(Request $request)
{
    ...
    try {
        ...
        $missingFields = [];
        if (empty($apiUrl)) $missingFields[] = 'API URL';
        if (empty($corporateId)) $missingFields[] = 'Corporate ID';
    ...
    try {
        ...
        try {
            if (\Schema::hasTable('attendances')) {
                $stats['today_punches'] = \App\Models\Attendance::whereDate('attendance_date', $today)->count();
                $stats['unique_users_today'] = \App\Models\Attendance::whereDate('attendance_date', $today)
...
private function createRealAttendanceRecords($dateRange, $validated)
{
    ...
    try {
        ...
        \Log::info('Fetching data from ETimeOffice API', [
            'from_date' => $fromDate,
            'to_date' => $toDate,
            'api_url' => $apiUrl
        ]);
        ...
        \Log::info('Raw ETimeOffice API Response', [
        ...
        \Log::info('ETimeOffice API Response Analysis', [
        ...
        \Log::info('Processing ETimeOffice punch data', [
 ...
 */
private function processAttendanceRecord(array $record, bool $testMode = false): array
{
    // Find student by employee code
    $student = \App\Models\Student::where('employee_code', $record['employee_code'])->first();
    
    if (!$student) {
        return [
            'action' => 'skipped',
            'reason' => 'Student not found with employee code: ' . $record['employee_code']
        ];
    }
    ...
    $status = $this->determineAttendanceStatus($punchTime);
...
private function parseLogHistory(): array
{
    try {
        ...
        foreach (array_reverse($lines) as $line) {
            if (strpos($line, 'ETimeOffice sync') !== false || 
...
private function createNewAttendance(Student $student, array $record, Carbon $punchDateTime, string $attendanceDate): void
{
    try {
        ...
        $status = $this->determineAttendanceStatus($punchDateTime->format('H:i:s'), $settings);
...
public function dashboard(Request $request)
{
    try {
        ...
        return view('admin.attendance.dashboard', compact(
            'selectedDate', 'todayStats', 'absentStudents', 'recentActivity',
            'batches', 'courses', 'batchId', 'courseId', 'weeklyTrend',
            'liveData', 'systemStatus', 'weeklyStats'
        ));
...
{
    // Base query for students - filter through batch to respect academic year filtering
    $studentsQuery = \App\Models\Student::where('status', 'active')
        ->whereHas('batch'); // This ensures we only get students with batches from selected academic year
    ...
    $attendanceQuery = \App\Models\Attendance::whereDate('attendance_date', $date);
...
{
    // Get all active students - filter through batch to respect academic year filtering
    $studentsQuery = \App\Models\Student::where('status', 'active')
        ->whereHas('batch') // This ensures we only get students with batches from selected academic year
        ->with(['batch.course']);
...
{
    $query = \App\Models\Attendance::with(['student.batch.course', 'faculty'])
...
{
    $lastAttendance = \App\Models\Attendance::where('student_id', $studentId)
...
public function bulkMarkPresent(Request $request)
{
    try {
        ...
        foreach ($request->student_ids as $studentId) {
            ...
            $existingAttendance = \App\Models\Attendance::where('student_id', $studentId)
                ->whereDate('attendance_date', $date)
                ->first();
