...
class ComponentPaymentController extends Controller
{
    ...
    public function exportActivityLog(Student $student)
    {
        ...
        return Excel::download(
            new PaymentActivityExport($activities, $student),
    ...
    public function componentPaymentReport(Request $request)
    {
        ...
        $categorySummary = FeeCategory::with([
            'studentFees' => function ($query) use ($startDate, $endDate) {
    ...
    public function create(Request $request)
    {
        ...
        if ($request->filled('student_id')) {
            ...
            // Fetch recent payments using Eloquent properly
            $recentPayments = Payment::where('student_id', $student->id)
    ...
    public function financialIndex(Request $request)
    {
        if ($request->ajax() && $request->filled('search')) {
            ...
            $students = Student::with(['studentFees', 'batch'])
            ...
            return view('admin.financial._search_results', compact('students'))->render();
        }
    ...
    /**
     * Batch Invoice Creation Stub
     */
    public function invoiceCreate()
    {
        return redirect()->back()->with('info', 'Batch Invoice Generation is coming soon.');
    }

    public function invoiceShow($invoice)
    {
        return redirect()->back()->with('info', 'Invoice viewing is coming soon.');
    }
}