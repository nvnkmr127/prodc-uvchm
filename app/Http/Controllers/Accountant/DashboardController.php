...
use App\Services\{DashboardService, DashboardDataService, ComponentPaymentService};
use App\Models\{Student, Payment, Batch, Course};
...
private function getSystemAlerts(): array
{
    ...
    private function getDefaulterTrends(): array 
    {
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $defaulterCount = Student::whereHas('componentPaymentItems', function($query) use ($monthStart, $monthEnd) {
    ...
    private function getCategoryTrends(): array { return []; }
    private function identifyImprovementAreas(array $efficiency): array { return []; }
    private function getBenchmarkComparison(array $efficiency): array { return []; }
    private function calculateRevenueVsTarget(array $financialData): array { return []; }
    private function getYearOverYearComparison(): array { return []; }
    private function calculateRevenueGrowth(array $trends): array { return []; }
    private function getRevenueByCategoryAnalysis(): array { return []; }
    private function generateRevenueForecast(array $trends): array { return []; }
    private function calculateProfitabilityMetrics(array $financialData): array { return []; }
    private function analyzeFeeStructureEfficiency(): array { return []; }
    private function getPricingOptimizationSuggestions(): array { return []; }
    private function getComponentUtilizationStats(): array { return []; }
    private function generateShortTermForecast(array $trends): array { return []; }
    private function generateLongTermForecast(array $trends): array { return []; }
    private function getScenarioAnalysis(array $trends): array { return []; }
    private function performRiskAssessment(): array { return []; }
    private function getStudentPaymentSegmentation(): array { return []; }
    private function getPaymentTimingAnalysis(): array { return []; }
    private function generateBehavioralInsights(array $behavior): array { return []; }
    private function getOutstandingTrends(): array { return []; }
    private function calculateRecoveryProbability(array $outstanding): array { return []; }
    private function generateActionRecommendations(array $outstanding): array { return []; }
    private function determineTrendDirection(array $growthRates): string { return 'stable'; }
    private function calculateVolatility(array $growthRates): float { return 0; }
    private function calculateGrowthConsistency(array $growthRates): float { return 0; }
    private function getMonthlyAchievement(float $monthlyTarget): float { return 0; }
    ...
    private function calculateTrendDirection(array $trends, string $field): string
    {
        ...
        for ($i = 1; $i < count($values); $i++) {
            ...
            elseif ($values[$i] < $values[$i-1]) $decreasing++;
        }
        
        if ($increasing > $decreasing) return 'increasing';
        elseif ($decreasing > $increasing) return 'decreasing';
        return 'stable';
    }
    ...
    private function getPerformanceGrade(float $collectionRate): string
    {
        ...
        if ($collectionRate >= 85) return 'Good';
        if ($collectionRate >= 70) return 'Average';
        if ($collectionRate >= 50) return 'Below Average';
        return 'Poor';
    }
    
    private function getPriorityLevel(float $collectionRate, float $totalBilled): string
    {
        if ($collectionRate < 50 && $totalBilled > 100000) return 'Critical';
        if ($collectionRate < 60 && $totalBilled > 50000) return 'High';
        if ($collectionRate < 70) return 'Medium';
        return 'Low';
    }
